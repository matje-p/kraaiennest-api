...
router.patch('/:boodschapId/:operation', async (req, res) => {
  ...
  try {
    ...
    if (operation === 'toggledone') {
      const { done, userDoneUuid, userDoneFirstname } = convertKeysSnakeToCamel(req.body);
      result = await req.db.query(
        'UPDATE boodschap_schema.boodschaps SET done = $1, user_done_uuid = $2, user_done_firstname = $3, date_done = CURRENT_TIMESTAMP WHERE boodschap_id = $4 RETURNING *',
        [done, userDoneUuid, userDoneFirstname,  boodschapId]
      );
    
    } else if (operation === 'remove') {
      const { userRemovedUuid, userRemovedFirstname } = convertKeysSnakeToCamel(req.body);
      result = await req.db.query(
        'UPDATE boodschap_schema.boodschaps SET removed = TRUE, user_removed_uuid = $1, user_removed_firstname = $2, date_removed = CURRENT_TIMESTAMP WHERE boodschap_id = $3 RETURNING *',
        [userRemovedUuid, userRemovedFirstname, boodschapId]
      );

    } else if (operation === 'edit') {
      const { item, userChangedUuid, userChangedFirstname } = convertKeysSnakeToCamel(req.body);
      result = await req.db.query(
        'UPDATE boodschap_schema.boodschaps SET item = $1, user_changed_uuid = $2, user_changed_firstname = $3, date_changed = CURRENT_TIMESTAMP, changed = TRUE WHERE boodschap_id = $4 RETURNING *',
        [item, userChangedUuid, userChangedFirstname, boodschapId]
      );
    } else {
